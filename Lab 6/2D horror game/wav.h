...
void CALLBACK RepeatCallback(HWAVEOUT hwo, UINT uMsg, DWORD_PTR dwInstance, DWORD dwParam1, DWORD dwParam2)
{
...
HWAV PlayWavFile(PCHAR filename, BOOL bLoop)
{
    ...
    if (pWav = (PWAV)LocalAlloc(LMEM_ZEROINIT, sizeof(WAV)))
    {
        ...
        {
            if (waveOutOpen(&pWav->hWaveOut, WAVE_MAPPER, &pWav->WaveFormatEx, (DWORD_PTR)RepeatCallback, (DWORD_PTR)pWav, CALLBACK_FUNCTION) != MMSYSERR_NOERROR)
            {
